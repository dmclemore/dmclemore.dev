...
const MazeGenerator: React.FC = () => {
	...
	const [lastSolveMethod, setLastSolveMethod] = useState<
		"manual" | "auto" | null
	>(null);
		...
		(x: number, y: number) => {
			if (isDragging) {
				...
				if (userPath.length === 0) {
					...
					const existingIndex = userPath.findIndex(
						pos => pos.x === x && pos.y === y
					);
					if (existingIndex === -1) {
	...
	const handleKeyDown = useCallback(
		(e: KeyboardEvent) => {
			...
			if (
				maze[newY] &&
				maze[newY][newX] &&
				isValidMove(currentPos, { x: newX, y: newY })
			) {
		...
		switch (gameState) {
			...
			case "victory":
				return (
					<VictoryScreen
						solveTime={
							solveEndTime && solveStartTime ? solveEndTime - solveStartTime : 0
						}
						onAutoSolve={handleAutoSolve}
						onResetMaze={handleResetMaze}
						onStartOver={handleStartOver}
