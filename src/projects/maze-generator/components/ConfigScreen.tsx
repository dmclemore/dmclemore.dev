...
const ConfigScreen: React.FC<ConfigScreenProps> = ({ config, onConfigComplete }) => {
  ...
  const [generationSpeed, setGenerationSpeed] = useState(config.generationSpeed);
  ...
  const getGridCellSize = (): { cellSize: number; gridStyle: React.CSSProperties } => {
  ...
  const handleGenerate = () => {
    ...
    const isStartOnEdge = finalStartX === 0 || finalStartX === finalWidth - 1 || 
                         finalStartY === 0 || finalStartY === finalHeight - 1;
                ...
                {Array.from({ length: width }, (_, x) => {
                  ...
                  return (
                    <div
                      ...
                      title={
                        isStart ? `Start: (${x}, ${y}) - Click to clear`
                        : isEnd ? `End: (${x}, ${y}) - Click to clear`
                        : isEdge ? `Click to set start or end position at (${x}, ${y})`
      ...
      <div className="w-full lg:w-64 xl:w-72 flex-shrink-0 bg-muted/30 rounded-lg p-3 sm:p-4 border border-border overflow-auto order-1 lg:order-2 max-h-64 lg:max-h-none">
        <div className="space-y-4 sm:space-y-6">
          <div className="hidden lg:block">
            <h2 className="text-xl font-bold text-foreground mb-4 text-center">Configure Your Maze</h2>
          </div>
          
          {/* Size Configuration */}
          <div className="bg-background p-4 rounded-lg border border-border">
            <h3 className="text-lg font-semibold text-foreground mb-4">Maze Size</h3>
          ...
          {/* Generation Speed */}
          <div className="bg-background p-4 rounded-lg border border-border">
            <h3 className="text-lg font-semibold text-foreground mb-4">Generation Speed</h3>
            <div className="flex items-center space-x-2 sm:space-x-4 min-w-0">
              <span className="text-xs sm:text-sm text-muted-foreground flex-shrink-0">Slow</span>
              <input
                type="range"
                min="10"
                max="200"
                value={210 - generationSpeed}
                onChange={(e) => setGenerationSpeed(210 - parseInt(e.target.value))}
                className="flex-1 min-w-0 h-2 bg-secondary rounded-lg appearance-none cursor-pointer accent-primary"
              />
              <span className="text-xs sm:text-sm text-muted-foreground flex-shrink-0">Fast</span>
            </div>
          ...
          <div className="bg-background p-4 rounded-lg border border-border">
            <h3 className="text-lg font-semibold text-foreground mb-3">Start & End Positions</h3>
            
            <div className="space-y-3">
              <div className="grid grid-cols-1 gap-3 text-sm">
                <div className="flex items-center space-x-2">
                  <div className="w-4 h-4 bg-green-500 rounded border border-border flex-shrink-0"></div>
                  <span className="text-foreground">Start Position (must be on outer edge)</span>
                </div>
                <div className="flex items-center space-x-2">
                  <div className="w-4 h-4 bg-red-500 rounded border border-border flex-shrink-0"></div>
                  <span className="text-foreground">End Position (can be anywhere)</span>
                </div>
              </div>
              ...
              <div className="text-center text-sm text-muted-foreground p-2 bg-muted/50 rounded">
                ...
                <p>Start: ({startX}, {startY}) → End: ({endX}, {endY})</p>
              </div>
              
              <div className="text-xs text-muted-foreground">
                <p>Click on the grid to set positions. Start must be on the outer edge, end can be placed anywhere including edges.</p>
              </div>
            </div>
          </div>
